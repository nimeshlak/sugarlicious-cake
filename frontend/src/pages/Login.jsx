  ...
  const onSubmitHandler = async (event) => {
    ...
    try {
      if(currentState === "Sign Up"){
        const response = await axios.post('https://sugarlicious-cake-backend.vercel.app/api/user/register', {name, email, password})
        ...
        const response = await axios.post('https://sugarlicious-cake-backend.vercel.app/api/user/login', {email,password})
        if(response.data.success){
          setToken(response.data.token)
          localStorage.setItem('token', response.data.token)
        }else{
